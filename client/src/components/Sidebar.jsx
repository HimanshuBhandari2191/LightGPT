...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-65 p-5 dark:bg-gradient-to-b 
    from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl 
    transition-all duration-500 max-md:absolute left-0 z-10 ${isMenuOpen ? 'top-0 w-3/4' : 'top-[-100%] w-0'} max-md:h-full`}>
      <img src={theme === 'dark' ? assets.logo_full : assets.logo_full_dark} alt="" className="w-32 h-8 mx-1 my-1" />
    ...
    <div>
      { 
        chats.filter((chat)=>chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()): chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
          ...
          key={chat.id} className='p-2 px-4 dark:bg-[#57317C]/10 border 
          border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer
          flex justify-between group'>
           <div>
            <p className='text-sm text-gray-800 dark:text-white truncate'>{chat.messages[0]?.content || chat.name}</p>
          

            <p className='text-xs text-gray-500 dark:text-gray-400'>{moment(chat.updatedAt).fromNow()}</p>
      ...
      {/*community Images*/}
      <div onClick={()=>{navigate('/community'); setIsMenuOpen(false)}} className="flex item-center gap-2 p-3 mt-4 border border-gray-300
      dark:border-white/15 rounded-md cursor-pointer hover:scale-110 transition-all">
      ...
      <div onClick={()=>{navigate('/credits');  setIsMenuOpen(false)}} className="flex item-center gap-2 p-3 mt-4 border border-gray-300
      dark:border-white/15 rounded-md cursor-pointer hover:scale-110 transition-all">
      ...
      {/*Theme Toggler*/}
         <div className="flex item-center justify-between gap-2 p-3 mt-4 border border-gray-300
      ...
      {/* User Info Section */}
        <div className="flex item-center gap-3 p-3 mt-4 border border-gray-300
      dark:border-white/15 rounded-md cursor-pointer group">
        <img src={assets.user_icon} alt="" className='w-7 rounded-full' />
          ...
          { user ? <img src={assets.logout_icon} className='h-5 hidden group-hover:block not-dark:invert cursor-pointer'/> : null}
      </div>
